...
public class Unit : MonoBehaviour
{
    ...
    public void animate(bool isDeath)
    {
        Transform unitRoot = objectInstance.transform.Find("HorseRoot");
        // Check if the HorseRoot child object is found
        if (unitRoot != null)
        {
            // Add the AnimationS script component to the HorseRoot child object
            AnimationS animationScript = unitRoot.gameObject.AddComponent<AnimationS>();
            animationScript.animateHorse(isDeath);
            // Optionally, you can initialize or modify properties of the added script component
            // For example:
            // animationScript.Initialize(parameters);

            Debug.Log("AnimationS script added to HorseRoot.");
        }
        else
        {
            Debug.LogError("HorseRoot child object not found!");
        }
    }
